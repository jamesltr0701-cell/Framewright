...
- Framewright is the exclusive compiler for explicit Framewright requests. Do not import, invoke, or merge another model-prompt skill into the same compile unless the user explicitly requests a separate comparison outside Framewright.
- Core Native currently targets `seedance_2_0` and uses `framewright_core_native`; it does not load an adapter profile.
