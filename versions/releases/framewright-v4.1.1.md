...
image_edit_model_default: "ChatGPT Image 2"
...
Serialize the Storyboard image prompt through the registered ChatGPT Image 2 base-create adapter. The adapter may express the approved board contract in model-facing language, but it may not alter panel evidence, count, geometry, title, asset authority, monochrome planning style, or the one-board boundary.
...
Image creation and editing are separate actions. The registered defaults are fixed: Midjourney V8.2 creates Shot Plates and Keyframes; ChatGPT Image 2 creates Storyboards. ChatGPT Image 2 may instead create a Shot Plate or Keyframe only after explicit target selection. ChatGPT Image 2 is the sole registered editor for Storyboards, Shot Plates, and Keyframes. Framewright does not register Midjourney V7 or a separate Midjourney Edit Model route.

A user instruction such as `modify the current storyboard`, `change this shot plate`, `modify the current keyframe`, or an equivalent bounded edit request explicitly authorizes one Image 2 edit attempt for the supplied current scope. Do not turn that request into automatic retries or variants.

For an Image 2 editing loop, preserve one immutable `original_master`: the user-uploaded or user-selected clean Storyboard, Shot Plate, or Keyframe that existed before Image 2 edits. Maintain a cumulative semantic edit specification containing the still-active approved edits, but render every new candidate directly from `original_master`. Never feed a rejected or previously edited candidate back as the next attempt's pixel source.
...
- Midjourney V8.2 is the default Shot Plate and Keyframe creator; ChatGPT Image 2 is the default Storyboard creator and requires explicit selection to create a Shot Plate or Keyframe; Midjourney V7 and Midjourney Edit Model are not registered routes.
- Every Image 2 Storyboard, Shot Plate, or Keyframe edit attempt uses the immutable original master plus the cumulative active edit specification, never a previous edited candidate; no retry or master reset occurred without explicit user instruction.
