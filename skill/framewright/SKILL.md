...
Default to prompt-artifact generation only. For a requested Framewright compilation, creating and saving the active stage's required `.txt` prompt artifact is part of normal compilation after the intake, stage, reference, and generation-unit boundary gates have been satisfied. It does not require a second file-creation authorization.
