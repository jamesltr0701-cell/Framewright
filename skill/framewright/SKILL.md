...
For Storyboard, Shot Plate, or Keyframe work, first read `references/keyframe_profiles/adapter_registry.yaml`, resolve the artifact kind and operation, and read only the registered profile that owns that route. Midjourney V8.2 is the default creator for Shot Plates and Keyframes. ChatGPT Image 2 is the default Storyboard creator and may create a Shot Plate or Keyframe only when the director explicitly selects it. ChatGPT Image 2 is the sole registered editor for Storyboards, Shot Plates, and Keyframes. Core owns artifact purpose, board or frozen-image structure, shot scope, look contract, continuity, and reference authority; the selected profile owns only model-facing image serialization and its Run Card. Never route to Midjourney V7 or a Midjourney Edit Model adapter.
